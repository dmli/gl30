...
public class YCUtils {
    ...
    public static int loadTexture(final Buffer data, final int width,final int height, final int type, final int usedTexId) {
        if(data == null)
            return NO_TEXTURE;
        int textures[] = new int[1];
        if (usedTexId == NO_TEXTURE) {
            ...
            GLES20.glTexParameterf(GLES20.GL_TEXTURE_2D, GLES20.GL_TEXTURE_WRAP_S, GLES20.GL_CLAMP_TO_EDGE);
            GLES20.glTexParameterf(GLES20.GL_TEXTURE_2D, GLES20.GL_TEXTURE_WRAP_T, GLES20.GL_CLAMP_TO_EDGE);
            GLES20.glTexImage2D(GLES20.GL_TEXTURE_2D, 0, GLES20.GL_RGBA, width, height, 0, GLES20.GL_RGBA, type, data);
        } else {
            GLES20.glBindTexture(GLES20.GL_TEXTURE_2D, usedTexId);
            GLES20.glTexSubImage2D(GLES20.GL_TEXTURE_2D, 0, 0, 0, width, height, GLES20.GL_RGBA, type, data);
            textures[0] = usedTexId;
        }
        return textures[0];
    }
     ...
     * @return 纹理ID
     */
    public static int createBlankTextureRGBAId(int width, int height, int type) {
        return createBlankTextureId(GLES20.GL_TEXTURE_2D, width, height, GLES20.GL_RGBA, type);
    }
     ...
     * @param format              GL_RGB/GL_RGBA/GL_LUMINANCE/GL_LUMINANCE_ALPHA
     * @param pixels              Buffer
     */
    public static void subTexture2D(int textureSerialNumber, int texId, int texWidth, int texHeight, int format, Buffer pixels) {
        subTextureBuffer(GLES20.GL_TEXTURE_2D, textureSerialNumber, texId, texWidth, texHeight, format, pixels);
    }
     ...
     * @param pixels              Buffer
     */
    public static void subTextureBuffer(int target, int textureSerialNumber, int texId, int texWidth, int texHeight, int format, Buffer pixels) {
        GLES20.glActiveTexture(textureSerialNumber);
        GLES20.glBindTexture(target, texId);
        GLES20.glTexSubImage2D(target, 0, 0, 0, texWidth, texHeight, GLES20.GL_RGBA, GLES20.GL_BYTE, pixels);
    }
     ...
     */
    public static int glGenBuffers(int arrayBufferType, float[] bufferArray) {
        ...
        if (bufferArray != null) {
            ...
            GLES20.glBindBuffer(arrayBufferType, 0);
        }
        return bufferIds[0];
    }

    public static int glGenBuffers(int arrayBufferType, int[] bufferArray) {
        int[] bufferIds = new int[1];
        GLES20.glGenBuffers(1, bufferIds, 0);
        if (bufferArray != null) {
            GLES20.glBindBuffer(arrayBufferType, bufferIds[0]);
            GLES20.glBufferData(arrayBufferType, bufferArray.length * 4, IntBuffer.wrap(bufferArray).position(0), GLES20.GL_STATIC_DRAW);
            GLES20.glBindBuffer(arrayBufferType, 0);
        }
        return bufferIds[0];
    }

    public static void glBufferSubData(int arrayBufferType, int bufferId, float[] bufferArray) {

        GLES20.glBindBuffer(arrayBufferType, bufferId);
        GLES20.glBufferSubData(arrayBufferType, 0, bufferArray.length * 4, FloatBuffer.wrap(bufferArray).position(0));
        GLES20.glBindBuffer(arrayBufferType, 0);
    }

    public static void glBufferSubData(int arrayBufferType, int bufferId, int[] bufferArray) {
        GLES20.glBindBuffer(arrayBufferType, bufferId);
        GLES20.glBufferSubData(arrayBufferType, 0, bufferArray.length * 4, IntBuffer.wrap(bufferArray).position(0));
        GLES20.glBindBuffer(arrayBufferType, 0);
    }

    public static void glBufferSubData(int arrayBufferType, int bufferId, short[] bufferArray) {
        GLES20.glBindBuffer(arrayBufferType, bufferId);
        GLES20.glBufferSubData(arrayBufferType, 0, bufferArray.length * 2, ShortBuffer.wrap(bufferArray).position(0));
        GLES20.glBindBuffer(arrayBufferType, 0);
    }
    ...
    public static void checkGLError(final String tag) {
        int loopCnt = 0;
        for (int err = GLES30.glGetError(); loopCnt < 32 && err != GLES30.GL_FALSE; err = GLES30.glGetError(), ++loopCnt) {
            String msg;
            switch (err) {
            ...
            Log.e("YCProgram", String.format("After tag \"%s\" glGetError %s(0x%x) ", tag, msg, err));
        }
    }
}
